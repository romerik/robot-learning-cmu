...
class RL_Trainer(object):
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            if self.log_video or self.log_metrics:
                ...
                avg, std, expert_avg, expert_std, bc_avg = self.perform_logging(
                ...
                # return avg, std, expert_avg, expert_std
                eval_returns.append(avg)
                std_returns.append(std)
                expert_returns.append(expert_avg)
                bc_returns.append(bc_avg)

        return eval_returns, std_returns, expert_returns, bc_returns
    ...
    def collect_training_trajectories(
        ...
        if itr == 0 and load_initial_expertdata is not None:
            with open(load_initial_expertdata, 'rb') as expertdata:
                loaded_paths = pickle.load(expertdata)
            return loaded_paths, 0, None
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for path in paths:
            ...
            # Create a new path dictionary with expert actions
            relabeled_path = {
                "observation": path["observation"],
                "image_obs": path.get("image_obs", None),
                "reward": path["reward"],
                "action": expert_actions,
                "next_observation": path["next_observation"],
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, training_logs):
        ...
        if self.log_metrics:
            ...
            if itr == 0:
                ...
                self.initial_return_std = np.std(train_returns)
                self.bc_initial_return = np.mean(eval_returns)
            
            logs["Accuracy"] = float(f"{(np.mean(eval_returns) / self.initial_return)*100:.2f}")
            logs["Initial_DataCollection_AverageReturn"] = self.initial_return
            logs["Initial_DataCollection_StdReturn"] = self.initial_return_std
            logs["Initial_DataCollection_AverageReturn_BC"] = self.bc_initial_return
            ...
            return logs['Eval_AverageReturn'], logs['Eval_StdReturn'], logs["Initial_DataCollection_AverageReturn"], logs["Initial_DataCollection_StdReturn"], logs["Initial_DataCollection_AverageReturn_BC"]
