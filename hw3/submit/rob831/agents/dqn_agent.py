...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        perform_random_action = np.random.random() < eps or self.t < self.learning_starts #TODO
        if perform_random_action:
            action = np.random.randint(self.num_actions) #TODO
        else:
            # HINT: Your actor will take in multiple previous observations ("frames") in order
                # to deal with the partial observability of the environment. Get the most recent 
                # `frame_history_len` observations using functionality from the replay buffer,
                # and then use those observations as input to your actor. 
            # We need to encode the recent observation to handle partial observability
            # Use the most recent observation to get action
            recent_obs = self.replay_buffer.encode_recent_observation()
            # The actor (ArgMaxPolicy) will handle conversion to tensor through the critic
