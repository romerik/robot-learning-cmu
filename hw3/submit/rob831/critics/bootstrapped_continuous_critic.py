...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        reward_n = ptu.from_numpy(reward_n) if not torch.is_tensor(reward_n) else reward_n
        ...
        # Perform target network updates
        for i in range(total_steps):
            # Recompute target values every num_grad_steps_per_target_update steps
            if i % self.num_grad_steps_per_target_update == 0:
                # Calculate V(s') by querying critic with next states
                next_values = self.forward(next_ob_no)
                
                # Compute target values: r + γV(s') with terminal state handling
                # Set V(s') = 0 for terminal states using (1 - terminal_n)
                target_values = reward_n + self.gamma * next_values * (1 - terminal_n)
                
                # Detach targets since we don't want to backprop through them
                target_values = target_values.detach()
            
            # Get current value predictions
            current_values = self.forward(ob_no)
            
            # Compute loss and update critic network
